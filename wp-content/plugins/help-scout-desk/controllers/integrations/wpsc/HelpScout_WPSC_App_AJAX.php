 ...
 */
class HelpScout_WPSC_App_AJAX extends HelpScout_WPSC_App {
	...
	public static function ajax_action() {
		...
		if ( ! current_user_can( 'manage_options' ) && ! wp_verify_nonce( $_REQUEST['nonce'], 'hs-wpsc-' . $_REQUEST[ self::APP_AJAX_HANDLER ] ) ) {
	...
	private static function handle_purchase_receipt_resend() {
		...
		wpsc_purchlog_resend_email( $purchase_id );
