		...
		<?php if ( ! empty( $current_conversations ) ) : ?>
			...
			<?php foreach ( $current_conversations as $key => $data ) : ?>
				<tr>
					...
					<?php /**/ ?>
					<td>
						<time datetime="<?php esc_attr_e( $data['createdAt'] , 'help-scout-desk' ) ?>"><?php echo date_i18n( get_option( 'date_format' ), strtotime( esc_attr__( $data['createdAt'] , 'help-scout-desk' ) ) ) ?></time>
					</td>
				</tr>
			<?php endforeach ?>
