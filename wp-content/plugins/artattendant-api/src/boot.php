...
namespace artattendant\artattendant_api;
...
class boot {
	...
	public function register_routes() {
		...
		$cb_class = new \artattendant\artattendant_api\routes\products( 'download', $base );
		...
		/**
		 * All Artworks, or query artworks by slug.
		 */
		register_rest_route( "{$root}/{$version}", '/artworks', array(
				array(
					'methods'         => \WP_REST_Server::READABLE,
					'callback'        => array( $cb_class, 'get_items' ),
					'args'            => array(
		...
		$cb_class = new \artattendant\artattendant_api\routes\docs( 'doc', $base );
		 ...
		 * Utility endpoints
		 */
		$cb_class = new \artattendant\artattendant_api\routes\util();
		/**
		 * Docs for all products or a specific product by slug/ID
		 */
