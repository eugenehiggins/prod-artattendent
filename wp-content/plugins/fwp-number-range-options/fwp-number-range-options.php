...
class FwpNumberRangeOptions {
	...
	function render( $params ) {
		...
		foreach( $choices as $choice ) {
			...
			$output .= '<div class="facetwp-'.$type . $selected . '"  name="facetwp_' . $params['facet']['name'] . '" data-value="' . $choice['range'] . '" data-facetwp-min="' . $choice['min'] . '" data-facetwp-max="' . $choice['max'] . '"' . $selected . '>' . $choice['label'] . ' <span class="facetwp-counter">(' . $choice['count'] . ')</span></div>';
		}
...
(function($) {
	...
	wp.hooks.addAction('facetwp/ready', function() {
		$(document).on('click', '.facetwp-type-number_range_options div', function() {
...
<?php
	...
	function index_row( $params, $class ) {
		...
		if (empty( $public_status ) && ( $public_status !=1 || $public_status !=3 ) ) return false;
